...
///
/// Lives in `Shared/` because it is rendered inside the `ScreenSpanReport`
/// extension (by `ChartReportView`). The grid's inputs are a fully-
/// formed `LifeGridData` value produced by `ProjectionCalculator` — it
/// does no Screen Time work itself and holds no per-app tokens — so it
